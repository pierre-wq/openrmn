...
def amazon_oauth_callback(code: str = "", state: str = "", error: str = "", error_description: str = ""):
    ...
    try:
        ...
        return HTMLResponse(f"<h2>❌ Profiles fetch failed</h2><pre>{e}</pre>", status_code=502)

    if not profiles:
        return HTMLResponse(
            "<h2>⚠️ Refresh token obtained but no EU profile found</h2>"
            "<p>Make sure the Amazon Ads account has at least one EU account.</p>",
            status_code=200,
        )
    ...
    <html><head><meta charset=utf-8><title>openRMN — Amazon connected</title>
...
def _wrap_real(fn, mode: str):
    try:
        ...
        raise HTTPException(502, f"Fetch {mode} failed: {e}")
...
async def _stream_claude(request: Request, system: str, user_payload: str):
    try:
        ...
        yield {"event": "error", "data": json.dumps({"message": f"anthropic lib missing: {e}"})}
        return
...
async def api_brief(
    ...
    async def event_gen():
        try:
            ...
            yield {"event": "error", "data": json.dumps({"message": f"Fetch {mode} failed: {e}"})}
            return
        df = apply_filters(df, prod_list, camp_list)
        if df.empty:
            yield {"event": "error", "data": json.dumps({"message": f"No data in {mode} mode for the selection — brief impossible."})}
            return
